...
impl Player {
    ...
    fn do_play(&mut self, pitch: Pitch, on: bool) {
    ...
    fn handle_play(&mut self, n: Option<u8>, note: Option<ReplNote>) {
        if let Some(i) = n
            && let Some(old) = self.state.notes.remove(&i)
        {
            println!("-     {old}");
            self.do_play(old.computed, false);
        }
    ...
    fn handle_command(&mut self, command: PromptCommand) {
        match command {
            PromptCommand::Reset => self.reset(),
            PromptCommand::Clear => self.clear(),
            ...
            PromptCommand::Play { n, note } => self.handle_play(n, note),
