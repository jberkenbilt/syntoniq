...
impl TestController {
    ...
    async fn send_key(&mut self, key: KeyData, on: bool) -> anyhow::Result<()> {
    ...
    async fn press_key(&mut self, key: KeyData) -> anyhow::Result<()> {
        self.send_key(key, true).await
    }

    async fn release_key(&mut self, key: KeyData) -> anyhow::Result<()> {
        self.send_key(key, false).await
    }

    async fn press_and_release_key(&mut self, key: KeyData) -> anyhow::Result<()> {
