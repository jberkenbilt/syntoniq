+++
title = "Keyboard: Getting Started"
weight = 10
sort_by = "weight"
+++

[This video accompanies [SYNTONIQ KEYBOARD -> Getting Started](../../keyboard/initialization/).]

[Present on the screen: a video of one of the controllers when in use, the web UI, and a terminal window.]

This video shows you how to start the Syntoniq keyboard application with the Novation Launchpad Pro Mark 3 and the HexBoard MIDI controller. I'll start the keyboard from the command line, select a layout, and play a few notes using the default Csound output mode. Then I'll hook it up to a synth and generate MIDI output. This video shows a web UI, a terminal, and videos of the actual keyboards, There's a lot of detail, so I recommend watching on a full-sized display.
...
Here's my Surge XT window. [Switch video focus to Surge XT window.] I'm running this as a stand-alone application in Linux, but it's available for mac and Windows as well. You can also run Surge XT inside a digital audio workstation, and if you don't like Surge, you can use this with any software that can receive input from a MIDI instrument. If you just start pressing keys now, there's a good chance you'll either hear nothing, the wrong note, or two notes at once. This is because the actual hardware itself is a MIDI device, and the Syntoniq keyboard is also a MIDI device. For this to work, you need to make sure you go into audio and midi options. Be sure to unselect the MIDI port for the actual device and select "Syntoniq Keyboard" as your active MIDI input. You can see that I've done that here. [Open audio options and show it.] For the patch, I've chosen the Analog Brass patch by Luna. If the audio sounds a bit fuzzy, it has to do with my screen capture program. I assure you it sounds perfectly clean through the actual speakers. Now when I press keys [press some keys], I hear sounds from this excellent patch. If you are accustomed to using MIDI keyboards through a synthesizer or audio workstation, you may prefer this as the sound quality will likely be much better than my "study tone" Csound instrument. Please keep in mind that Csound is an extremely capable tool, and you should not judge the tool by the quality of the default sound used by the Syntoniq keyboard. If you want better sounds with the keyboard, use MIDI. For the Syntoniq language, you can use your own Csound instruments in place of the study tone, or you can just use MIDI.
